...
class LoadingProfileFragment: Fragment() {
    ...
    suspend fun loginProfile(){
        ...
        if(usrType == "dogs"){
            ...
            action = LoadingProfileFragmentDirections.actionLoadingProfileFragmentToProfileDogFragment(usr, picture, awayMatches.toTypedArray())
        } else {
            ...
            val uri = storage.child(usr.pictures[0]).downloadUrl.await()
            val picture = getProfilePictureFromURI(uri)
            action = LoadingProfileFragmentDirections.actionLoadingProfileFragmentToProfileSitterFragment(usr, picture, awayMatches.toTypedArray())
        }
